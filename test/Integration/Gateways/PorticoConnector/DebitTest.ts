...
test("debit sale", async (t) => {
  ...
  const response = await track.charge(17.01)
    .withCurrency("USD")
...
test.failing("debit add value", async (t) => {
  ...
  const response = await track.addValue(15.01)
    .withCurrency("USD")
    .withAllowDuplicates(true)
    .execute();
...
test("debit refund", async (t) => {
  ...
  const response = await track.refund(16.01)
    .withCurrency("USD")
    .withAllowDuplicates(true)
    .execute();
...
test("debit reverse", async (t) => {
  ...
  const response = await track.reverse(17.01)
    .withCurrency("USD")
    .withAllowDuplicates(true)
    .execute();
...
test("debit cannot refund from transaction id only", (t) => {
  ...
  const error = t.throws(() => {
    Transaction.fromId("1234567890", PaymentMethodType.Debit).refund().withCurrency("USD").execute();
  }, UnsupportedTransactionError);
