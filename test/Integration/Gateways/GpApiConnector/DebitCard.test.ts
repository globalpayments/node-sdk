...
test("debit sale swipe", async () => {
  ...
  assertTransactionResponse(response, "SUCCESS", TransactionStatus.CAPTURED);
});
...
test("debit reverse", async () => {
  ...
  assertTransactionResponse(transaction, "SUCCESS", TransactionStatus.CAPTURED);

  const response = await transaction.reverse().withCurrency("USD").execute();

  assertTransactionResponse(response, "SUCCESS", TransactionStatus.REVERSED);
});
