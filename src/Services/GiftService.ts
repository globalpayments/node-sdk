...
export class GiftService {
  ...
  public activate(amount?: number | string) {
    return new AuthorizationBuilder(TransactionType.Activate).withAmount(
      amount,
    );
  }

  public addValue(amount?: number | string) {
    return new AuthorizationBuilder(TransactionType.AddValue).withAmount(
      amount,
    );
  }

  public addAlias(phoneNumber: string) {
    return new AuthorizationBuilder(TransactionType.Alias).withAlias(
      AliasAction.Add,
      phoneNumber,
    );
  }
