...
export class GpEcomManagementRequestBuilder
  ...
  public buildRequest(
    ...
    if (builder.alternativePaymentType !== null) {
      ...
      if (builder.transactionType == TransactionType.Confirm) {
        const paymentMethodDetails = subElement(
          request,
          "paymentmethoddetails",
        );
        const apmResponse = builder.paymentMethod.alternativePaymentResponse;
        if (
          builder.alternativePaymentType == AlternativePaymentType.Paypal &&
          apmResponse
        ) {
          subElement(paymentMethodDetails, "Token").append(
            cData(apmResponse.sessionToken),
          );
          subElement(paymentMethodDetails, "PayerID").append(
            cData(apmResponse.providerReference),
          );
        }
      }
    }
